...
assert abs(sum([v for k, v in SKILLS_SUB_WEIGHTS.items() if k != "penalty_deduction"]) - 1.0) < 1e-6, \
    "Skills sub-weights (excluding penalty_deduction) must sum to 1.0"
...
def validate_weights() -> bool:
    ...
    assert abs(sum(EXPERIENCE_SUB_WEIGHTS.values()) - 1.0) < 1e-6, \
        "Experience sub-weights must sum to 1.0"
    skills_sum = sum([v for k, v in SKILLS_SUB_WEIGHTS.items() if k != "penalty_deduction"])
    assert abs(skills_sum - 1.0) < 1e-6, \
        f"Skills sub-weights (excluding penalty_deduction) must sum to 1.0, got {skills_sum}"
    assert abs(sum(ALIGNMENT_SUB_WEIGHTS.values()) - 1.0) < 1e-6, \
        "Alignment sub-weights must sum to 1.0"
    assert abs(sum(BEHAVIORAL_WEIGHTS.values()) - 1.0) < 1e-6, \
        "Behavioral sub-weights must sum to 1.0"
