...
def run_stage2_pipeline(
    ...
    for rank_idx, candidate in enumerate(top_candidates, 1):
        ...
        # Generate deterministic reasoning based on computed component scores
        features_dict = {
            "experience": candidate.component_scores.experience_score,
            "skills": candidate.component_scores.skills_score,
            "behavior": candidate.component_scores.behavioral_score,
            "semantic": candidate.component_scores.semantic_alignment_score,
            "penalty": candidate.component_scores.anomaly_penalty
        }
        candidate.reasoning = generate_reasoning(features_dict, candidate.final_score)

    output_path = os.path.join(settings.PROJECT_ROOT, "data", "outputs", "stage2_top5000.csv")
    export_stage2_to_csv(top_candidates, output_path)
...
if __name__ == "__main__":
    ...
    Logger.info("Starting Stage-2 Reranking Pipeline Standalone Execution...")
    try:
        output_csv = os.path.join(settings.PROJECT_ROOT, "data", "outputs", "stage2_top5000.csv")
        ranked_candidates = run_stage2_pipeline()
        
        Logger.info("Stage-2 Reranking Pipeline completed successfully.")
        print(f"Total number of ranked candidates: {len(ranked_candidates)}")
        print(f"Output CSV path: {output_csv}")
    except Exception as e:
        Logger.error(f"Stage-2 Pipeline execution failed: {e}")
        sys.exit(1)
